...
The first goal for this course, is exact solutions to polynomials, for polynomials in low degree with coefficients in $\bC$, we already know some nice formulas.
...
This example is familiar even from highschool. The next step up in degree, already makes things significantly more challenging.
...
While the quadratic equation is pretty easy to derive, the unassuming undergrad might struggle to discover the \textit{trick} for the cubic formula. ``The best way to appreciate this is to go to a deserted island with no food, the only way to leave is once you have solved the problem. Some of you will die of hunger on that island, but at least you will have learned the problem is tough''.

Galois theory is a method for translating information from the baroque theory of fields, to the much simpler theories of finite groups and linear algebra. We will see that the \textbf{Galois group} associated to this polynomial is in general $S_3$ which is solvable, the composition series will show us how to produce the above formula.

There is a similar method for the quartic, which comes from the fact that $S_4$ is also solvable.
...
\begin{definition}
    $k\subset K$ is \textbf{normal} if for all $k\subset \Omega$ and $x_1,x_2\in\Emb_k(K,\Omega)$ there exists a $\varphi \in \Aut(K/k)$ such that $x_1=x_2\varphi$
\end{definition}
...
\begin{proof}[Proof of A4]
    We will go by induction on $[K:k]$ the bases case has $K=k$ so taking $\Omega = L$ works. Now suppose $[K:k]>1$, then there exists some $\alpha\in K\setminus k$, this will have a minimal polynomial $f\in k[X]$, let $L'$ be the splitting field of $f\in L[X]$, then $\Emb_k(k(\alpha),L') = \{\text{roots of $f$ in $L'$}\}$ which must be nonempty as $f$ is nontrivial. THus we can form the daigram:\[
    \begin{tikzcd}[row sep = small, column sep = small]
        & & K \\
        ...
        & L \arrow[ur, phantom, sloped, "\subset"]
    \end{tikzcd}
    \] where by the tower law $[K:k(\alpha)]<[K:k]$ so, by induction, we're done.
\end{proof}
...
\begin{lemma}
    ...
    \rho : \Emb_k(K,\Omega)\rightarrow \Emb_k(F,\Omega)
...
A striking consequence of this corollary is that any irreducible $f\in k[X]$ either has no roots in $K$, or splits completely! Indeed if $f$ has any root $a\in K$ then let $\Omega$ be the splitting field of $f$ in $K$, we can form the tower $k\subset k(a)\subset K \subset \Omega$ and we have the isomorphism: \[
    \{\text{roots of $f$ in $\Omega$}\} \cong \Emb_k(k(a),\Omega) \cong \Emb_k(k(a),K) \cong \{\text{roots of $f$ in $K$}\}
\] 
